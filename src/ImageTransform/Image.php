...
class Image
{
    ...
    {
        $defaults = array('filename' => 'Untitled.png', 'mime_type' => 'image/png', 'width' => 100, 'height' => 100);
    ...
    public function load($filename, $mime = '')
    {
        if (file_exists($filename) && is_readable($filename))
        {
            ...
            if ('' == $mime)
            {
                throw new \ImageTransform\Exception(sprintf('You must either specify the MIME type for file %s or enable mime detection', $filename));
            }
        ...
        throw new \ImageTransform\Exception(sprintf('Unable to load %s. File does not exist or is unreadable', $filename));
    }
    ...
    public function saveAs($filename, $mime = '')
    {
        ...
        if (!$mime)
        {
            throw new \ImageTransform\Exception(sprintf('Unsupported file %s', $filename));
        }
    ...
    {   
        ...
        {
            throw new \ImageTransform\Exception(sprintf('Unsupported transform %s. Cannot find %s adapter or generic transform class', $name, $this->getAdapter()->getAdapterName()));
        }
...
//        }
    ...
    protected function createAdapter($name)
    {
        ...
        {
            throw new \ImageTransform\Exception(sprintf('Unsupported adapter: %s', $adapter_class));
        }
