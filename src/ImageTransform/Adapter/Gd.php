...
namespace ImageTransform\Adapter;
...
class Gd extends \ImageTransform\Adapter
{
    ...
    public function __construct()
    {
        ...
        {
            throw new \ImageTransform\Exception('The image processing library GD is not enabled. See PHP Manual for installation instructions.');
        }
    }
    ...
    public function load($filename, $mime)
    {
        ...
        {
            throw new \ImageTransform\Exception(sprintf('Cannot load file %s as %s is an unsupported file type.', $filename, $mime));
        }
    }
    ...
    protected function __output($to_file = false, $filename = '')
    {
        ...
        if (array_key_exists($mime, $this->creators))
        {
            ...
            switch ($mime)
            {
                ...
                default:
                    throw new \ImageTransform\Exception(sprintf('Cannot convert as %s is an unsupported type', $mime));
            }
        }
        else
        {
